...
- ✅ **`apps/sample-app` couldn't build on Vercel at all — "Module not found: Can't resolve '@fip/sdk'".** Root cause: Vercel's `Root Directory = apps/sample-app` means it only ever runs `pnpm run build` scoped to that one package — it never triggers `@fip/sdk`'s own build step, so `dist/` never gets created (this only ever worked locally because I'd manually built the SDK first). Tried a `prebuild` hook first — pnpm doesn't auto-run npm-style pre/post script hooks unless `enable-pre-post-scripts` is set in `.npmrc` (it isn't here), so that silently did nothing. Fixed by chaining the SDK build directly into `apps/sample-app`'s own `build` script instead (config-independent). Verified by reproducing the exact failure locally first (deleted `packages/sdk/dist`, ran `pnpm run build` scoped only to `apps/sample-app`, same error), then confirming the fix resolves it under the same conditions.
- ✅ **`/demo` page added to `apps/sample-app`** — for live demos, not organic-traffic-dependent. 4 scenario buttons: trigger a real JS error (genuine throw, caught by the SDK's own listener), simulate a slow page (6 synthetic LCP samples 4.5-7s, crosses the LCP-regression alert threshold), simulate a slow/failing API (5 payment calls, one 504 timeout), simulate an error burst (12 errors, crosses the error-spike alert threshold). Plus a "Check for alerts now" button — alerts don't evaluate automatically on this deployment (no cron wired up), so this proxies through a new server-side-only route (`apps/sample-app/src/app/api/check-alerts/route.ts`, holds `FIP_ALERT_CHECK_SECRET` off the client) to the dashboard's protected `/api/alerts/check`. Verified end-to-end locally: each scenario's payload validated against the real ingest schema (`accepted: N, rejected: 0` for all 3 batch scenarios), and the check-alerts proxy actually fired both `error_spike` and `lcp_regression` alerts from the synthetic data, confirmed in the `Alert` table. New env vars needed on the sample-app deployment: `FIP_DASHBOARD_ORIGIN`, `FIP_ALERT_CHECK_SECRET` (must match the dashboard's `AUTH_SECRET`). Note: alerts have a 1-hour cooldown per (appId, kind) — documented on the page itself so a repeat demo within an hour doesn't look broken when the same alert kind doesn't refire.
- ✅ **`BundleReport` seed gap fixed** — `db:seed` never populated this table (the original local dashboard's bundle data came from manual one-off `curl` commands during dev, never captured in the seed script or replicated to Neon). Added bundle sizes to `scripts/seed.ts` for future fresh databases, plus a one-off `scripts/seed-bundles.ts` backfill (since re-running full `db:seed` would have duplicated the 1,323 already-seeded events — only `App`/`Release` there are upserts). Verified: all 3 releases now show growing bundle sizes (376KB → 438KB) in Neon, matching the LCP-regression story already told by the rest of the seeded data.
